...
class AuthController
{
    ...
    {
        $provider = (string) ($_GET['provider'] ?? 'google');
        $userId = User::findOrCreateSocial($provider);
        $_SESSION['user_id'] = $userId;
        flash('success', 'Zalogowano przez konto spolecznosciowe (wersja demonstracyjna).');
        redirect_to('quizzes.index');
    }

    public function language(): void
    {
        $lang = (string) ($_GET['lang'] ?? 'pl');
    ...
    public function settings(): void
    {
        ...
        $badges = Attempt::badgesForUser((int) $user['id']);

        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $theme = (string) ($_POST['preference_theme'] ?? 'light');
            $language = (string) ($_POST['preference_language'] ?? 'pl');
            User::updatePreference((int) $user['id'], $theme, $language);
            $_SESSION['lang'] = $language;
            flash('success', 'Preferencje zostaly zapisane.');
            redirect_to('auth.settings');
        }

        view('auth/settings', compact('user', 'attempts', 'badges'));
    }
}
