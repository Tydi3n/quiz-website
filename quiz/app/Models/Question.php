...
class Question
{
    ...
    public static function create(int $quizId, array $data): int
    {
        $stmt = Database::pdo()->prepare('
            INSERT INTO questions (quiz_id, type, question_text, points, correct_text_answer)
            VALUES (:quiz_id, :type, :question_text, :points, :correct_text_answer)
        ');
        $stmt->execute([
            'quiz_id' => $quizId,
            'type' => $data['type'],
    ...
    public static function update(int $id, array $data): void
    {
        $stmt = Database::pdo()->prepare('
            UPDATE questions
            SET type = :type, question_text = :question_text, points = :points, correct_text_answer = :correct_text_answer
            WHERE id = :id
        ');
        $stmt->execute([
            'id' => $id,
