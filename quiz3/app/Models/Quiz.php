    ...
    public static function all(array $filters = [], ?array $viewer = null): array
    {
        ...
        if (!empty($filters['search'])) {
            $where[] = '(
                ...
                OR u.username LIKE :search_author
                OR EXISTS (
                    SELECT 1
                    FROM quiz_categories sqc
                    JOIN categories sc ON sc.id = sqc.category_id
                    WHERE sqc.quiz_id = q.id AND sc.name LIKE :search_category
                )
            )';
